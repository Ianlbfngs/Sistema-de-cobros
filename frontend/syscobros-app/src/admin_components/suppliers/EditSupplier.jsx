...
function EditSupplier() {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            if (backendOK) {
                try {
                    const resultSupplier = await axios.get(`http://localhost:8080/api/suppliers/${idSupplier}`, { headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") } });
                    setSupplier(resultSupplier.data);
                } catch (error) {
                    console.error("Error fetching the supplier with id '" + idSupplier + "' :", error);
                    if (error.status === 404) {
                        alert("El proveedor con id '" + idSupplier + "' no existe en el sistema");
                    }
                    alert("Error intentando obtener el proveedor con id '" + idSupplier + "'");
    ...
    const onSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:8080/api/admin/suppliers/update/${idSupplier}`, supplier, { headers: { Authorization: 'Bearer ' + localStorage.getItem('jwtToken') } });
            showSuccessAlert();
        ...
        <div className='container'>
            ...
            {visibleSuccessAlert && (
                <div className="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 " role="alert">
                    ...
                    <div>
                        <span className="font-medium">Información del proveedor actualizada correctamente</span>
                    </div>
                </div>
            )}
            {(errorMessage !== "") && (
                <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 " role="alert">
                    ...
                    <div>
                        <span className="font-medium">No fue posible actualizar la información del proveedor: {errorMessage}</span>
                    </div>
                </div>
            )}
            <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                <form className="space-y-6" onSubmit={(e) => onSubmit(e)} >
                    <div>
                        ...
                        <div className="mt-2">
                            <input type="text" name="id" id="id" value={idSupplier} disabled className="block w-full rounded-md bg-gray-100 px-3 py-1.5 text-base text-gray-500 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-none sm:text-sm/6 cursor-not-allowed" />
                        </div>
                    </div>
                    <div>
                        <div className="flex items-center justify-between">
                    ...
                    <div>
                        ...
                        <div className="mt-2">
                            <input type="text" name="cuit" id="cuit" value={cuit} maxLength={11} onChange={onInputChangeSupplier} className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-500 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" />
                        </div>
                    </div>
                    <div>
                        <div className="flex items-center justify-between">
