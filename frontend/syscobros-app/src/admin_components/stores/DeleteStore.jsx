...
function DeleteStore() {
  ...
  const [foundStore, setFoundStore] = useState(true);
  ...
  useEffect(() => {
    ...
    const obtainStoreToDelete = async () => {
      try {
        const resultStore = await axios.get('http://localhost:8080/api/stores/' + id, {
          headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
        });
        setStore(resultStore.data);
        setFoundStore(true);
      } catch (error) {
        console.error("Error fetching the store with id " + id + " :", error);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.put('http://localhost:8080/api/admin/stores/delete/' + id, null, {
        headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
      });
      showSuccessAlert();
  ...
  function renderStore() {
    try {
        ...
        <table className="w-full text-sm text-gray-400">
          ...
          <tbody>
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 text-center">
              <th scope='row' className="px-6 py-3">{store.id}</th>
              <td scope="row" className="px-6 py-3">{store.name || "N/A"}</td>
              <td scope="row" className="px-6 py-3">{store.address}</td>
      ...
      console.error("Error loading the store to delete:", error);
      return (
        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
          <td colSpan={5} className='text-center p-3 text-red-400'  >Error cargando el local</td>
        </tr>
      )
    }
  }
    ...
    <div className='container'>
      ...
      {visibleSuccessAlert && (
        <div className="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 " role="alert">
          ...
          <div>
            <span className="font-medium">Local eliminado exitosamente</span>
          </div>
        </div>
      )}
      {(errorMessage !== "") && (
        <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 " role="alert">
          ...
          <div>
            <span className="font-medium">No fue posible eliminar el local solicitado: {errorMessage}</span>
          </div>
        </div>
      )}
              ...
              <div className="w-full flex justify-center">
                <div className="w-full max-w-md">
                  <div className="flex items-center justify-between">
                    <label htmlFor="idToDelete" className="block text-sm/6 font-medium text-gray-900">Id del local a eliminar</label>
                  </div>
                  <div className="mt-2">
            ...
            <div className="space-y-6">
              <div className="w-full flex justify-center">
                <div className="w-full max-w-md">
                  ...
                  {!foundStore &&
                    (
                      <span className='text-red-500'>* No se encontro ningun local con el ID solicitado</span>
                    )
                  }
                  <div className="mt-2">
