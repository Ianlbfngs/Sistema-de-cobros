...
function DeleteProduct() {
  ...
  useEffect(() => {
    ...
    const obtainProductToDelete = async () => {
      try {
        const resultProduct = await axios.get('http://localhost:8080/api/products/' + productCode, {
          headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
        });
        setProduct(resultProduct.data);
        setFoundProduct(true);
      } catch (error) {
        console.error("Error fetching the product with the code " + productCode + " :", error);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.put('http://localhost:8080/api/admin/products/delete/' + productCode, null, {
        headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
      });
      showSuccessAlert();
      ...
      return (
        <table className="w-full text-sm text-gray-400">
          <thead className="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400 text-center">
            <tr>
              <th scope="col" className="px-6 py-3">Código de Producto</th>
          ...
          <tbody>
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 text-center">
              <th scope='row' className="px-6 py-3">{product.productCode}</th>
              <td scope="row" className="px-6 py-3">{product.supplier?.companyName || "N/A"}</td>
              <td scope="row" className="px-6 py-3">{product.name}</td>
              <td scope="row" className="px-6 py-3">{product.price}</td>
              <td scope="row" className="px-6 py-3">{product.description}</td>
            </tr>
          </tbody>
        </table>
      )
    } catch (error) {
      console.error("Error loading the product to delete:", error);
      return (
        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
          <td colSpan={5} className='text-center p-3 text-red-400'  >Error cargando el producto</td>
        </tr>
      )
    }
  }
    ...
    <div className='container'>
      ...
      {visibleSuccessAlert && (
        <div className="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 " role="alert">
          ...
          <div>
            <span className="font-medium">Producto eliminado exitosamente</span>
          </div>
        </div>
      )}
      {(errorMessage !== "") && (
        <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 " role="alert">
          ...
          <div>
            <span className="font-medium">No fue posible eliminar el producto solicitado: {errorMessage}</span>
          </div>
        </div>
      )}
              ...
              <div className="w-full flex justify-center">
                <div className="w-full max-w-md">
                  <div className="flex items-center justify-between">
                    <label htmlFor="productCode" className="block text-sm/6 font-medium text-gray-900">Código del producto a eliminar</label>
                  </div>
                  <div className="mt-2">
                    <input type="text" name="productCode" id="productCode" value={productCode} onChange={onInputChangeCode} required disabled className="block w-full rounded-md bg-gray-100 px-3 py-1.5 text-base text-gray-500 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-none sm:text-sm/6 cursor-not-allowed" />
                  </div>
                </div>
              </div>
              {renderProduct()}
            ...
            <div className="space-y-6">
              <div className="w-full flex justify-center">
                <div className="w-full max-w-md">
                  ...
                  {!foundProduct &&
                    (
                      <span className='text-red-500'>* No se encontro ningun producto con el código indicado</span>
                    )
                  }
                  <div className="mt-2">
                    <input type="text" name="productCode" id="productCode" value={productCode} onChange={onInputChangeCode} required className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-500 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" />
                  </div>
                </div>
              </div>
              <div className="flex gap-2">
