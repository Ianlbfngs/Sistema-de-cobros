...
function AddProduct() {
  ...
  const { productCode, supplier, name, price, description } = product;
  const [productCodeInUse, setProductCodeInUse] = useState(false);
  ...
  useEffect(() => {
    const obtainProductCodeInUse = async () => {
      try {
        await axios.get('http://localhost:8080/api/products/' + productCode, {
          headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
        });
        setProductCodeInUse(true);
      } catch (error) {
        if (error?.status === 404) {
          setProductCodeInUse(false);
        } else {
  ...
  const onInputChangeProduct = (e) => {
    ...
    if (name === "suppliersDdl") {
      ...
      let priceValue = value.replaceAll(",", ".");
      setProduct(prev => ({
        ...prev,
        [name]: priceValue
      }));
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    setErrorMessage("");
    if (! await verifyBackendStatus(setApiOnline) || productCodeInUse) return;
    try {
      await axios.post(`http://localhost:8080/api/admin/products/add`, product, { headers: { Authorization: 'Bearer ' + localStorage.getItem('jwtToken') } });
      showSuccessAlert();
    } catch (error) {
      console.error(error);
      setErrorMessage(
    ...
    <div className='container'>
      ...
      {(errorMessage !== "") && (
        <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 " role="alert">
          ...
          <div>
            <span className="font-medium">No fue posible agregar el producto: {errorMessage}</span>
          </div>
        </div>
      )}
      <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
        <form className="space-y-6" onSubmit={(e) => onSubmit(e)} >
          <div>
            ...
            {productCodeInUse &&
              (
                <span className='text-red-500'>* El código ya esta en uso</span>
              )
            }
            <div className="mt-2">
              <input type="text" name="productCode" id="productCode" value={productCode} maxLength={8} onChange={onInputChangeProduct} required className="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-500 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between">
