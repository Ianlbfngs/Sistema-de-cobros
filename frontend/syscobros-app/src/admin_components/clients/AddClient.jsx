...
function AddClient() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:8080/api/admin/clients/add`, client, { headers: { Authorization: 'Bearer ' + localStorage.getItem('jwtToken') } });
      showSuccessAlert();
    ...
    <div className='container'>
      ...
      {visibleSuccessAlert && (
        <div className="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 " role="alert">
          ...
          <div>
            <span className="font-medium">Cliente agregado exitosamente</span>
          </div>
        </div>
      )}
      {(errorMessage !== "") && (
        <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 " role="alert">
          ...
          <div>
            <span className="font-medium">No fue posible agregar el cliente: {errorMessage}</span>
          </div>
        </div>
      )}
