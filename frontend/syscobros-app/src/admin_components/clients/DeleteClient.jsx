...
function DeleteClient() {
  ...
  useEffect(() => {
    ...
    const obtainClientToDelete = async () => {
      try {
        const resultClient = await axios.get('http://localhost:8080/api/admin/clients/' + id, {
          headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
        });
        setClient(resultClient.data);
        setFoundClient(true);
      } catch (error) {
        console.error("Error fetching the client with id " + id + " :", error);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      await axios.put('http://localhost:8080/api/admin/clients/delete/' + id, null, {
        headers: { Authorization: 'Bearer ' + localStorage.getItem("jwtToken") }
      });
      showSuccessAlert();
        ...
        <table className="w-full text-sm text-gray-400">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 text-center">
            <tr>
              ...
              <th scope="col" className="px-15 py-3">Apellido</th>
              <th scope="col" className="px-15 py-3">Teléfono</th>
              <th scope="col" className="px-15 py-3">Correo</th>
              <th scope="col" className="px-15 py-3">Dirección</th>
            </tr>
          </thead>
          <tbody>
            <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 text-center">
              <th scope='row' className="px-6 py-3">{client.id}</th>
              <td scope="row" className="px-6 py-3">{client.name}</td>
              <td scope="row" className="px-6 py-3">{client.surname}</td>
              <td scope="row" className="px-6 py-3">{client.phoneNumber || "N/A"}</td>
              <td scope="row" className="px-6 py-3">{client.email || "N/A"}</td>
              <td scope="row" className="px-6 py-3">{client.address || "N/A"}</td>
            </tr>
          </tbody>
        </table>
      )
    } catch (error) {
      console.error("Error loading the client to delete:", error);
      return (
        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
          <td colSpan={6} className='text-center p-3 text-red-400'  >Error cargando el cliente</td>
        </tr>
      )
    }
  }
    ...
    <div className='container'>
      ...
      {visibleSuccessAlert && (
        <div className="flex items-center p-4 mb-4 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 " role="alert">
          ...
          <div>
            <span className="font-medium">Cliente eliminado exitosamente</span>
          </div>
        </div>
      )}
      {(errorMessage !== "") && (
        <div className="flex items-center p-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 " role="alert">
          ...
          <div>
            <span className="font-medium">No fue posible eliminar el cliente solicitado: {errorMessage}</span>
          </div>
        </div>
      )}
              ...
              <div className="w-full flex justify-center">
                <div className="w-full max-w-md">
                  <div className="flex items-center justify-between">
                    <label htmlFor="idToDelete" className="block text-sm/6 font-medium text-gray-900">Id del cliente a eliminar</label>
                  </div>
                  <div className="mt-2">
            ...
            <div className="space-y-6">
              <div className="w-full flex justify-center">
                <div className="w-full max-w-md">
                  ...
                  {!foundClient &&
                    (
                      <span className='text-red-500'>* No se encontro ningun cliente con el ID solicitado</span>
                    )
                  }
                  <div className="mt-2">
